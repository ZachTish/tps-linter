...
Preserved from the README at 0.8.0, during the 2026-09-16 documentation cleanup. This includes earlier release notes, detailed contracts, and historical validation claims. Earlier setup steps or limitations may have been superseded. Start with [the current README](README.md), current source, and the tagged release. Relative source links remain rooted in this repository.

---

## 0.8.0 — Conservative triggers and frontmatter tags

Clean notes retains explicit-save linting, with page-focus linting now separately opt-in and off by default on upgrade. Automatic filename renaming remains disabled. Move tags to frontmatter is also opt-in: it merges standalone tag-only body lines into the tags field, deduplicating case-insensitively. Tags in tasks, prose, fences, indented content, and protected HTML/comment content are preserved. Invalid YAML or unsupported tags values are unchanged. The move-tags-to-frontmatter rule honors note-level disable controls. Both new options live beside existing Clean notes controls; the four destinations and other actions remain. Rules are shared vault configuration, not device-local preferences.

Verified in the reloaded test vault with a synthetic Inbox note: page focus left the note unchanged with focus linting disabled; Cmd-S moved standalone tags into frontmatter without altering prose, task, or fenced tags. Original settings were restored and the fixture was moved to _archive. Focused regressions cover invalid YAML, idempotence, disabled rules, settings migration, and the full cleaner. Shared rule preferences are described accurately without a misleading device-local claim. Full declared npm test and a separate final npm run build are required for the versioned artifacts; the stable build deploys only shipped files to Obsidian Plugin Test Vault. Hot Reload plus explicit test-plugin reload was used for UI verification. Release notes record final validation and SHA-256 hashes. Production installation remains the user’s BRAT pull.
...
Add the public repository `ZachTish/tps-linter` to BRAT and track `Latest`, or freeze the exact numeric release `0.7.6`. The release attaches BRAT's required `main.js`, `manifest.json`, and complete `styles.css` artifacts.
...
- **Lint on explicit save or page focus** is enabled by default. TPS Linter schedules the active Markdown page only when the standard platform save gesture (Cmd-S on Apple devices and Ctrl-S elsewhere) originates inside its source editor, focus enters that page from outside its view, or its Obsidian window regains focus while that page still owns focus. It waits 500 ms, coalesces repeated triggers, rechecks the live note and editor buffer, and applies enabled content rules only while the same saved page remains active and eligible.

Automatic linting is content-only: it never plans or applies a filename change. When the loaded TPS Global Context Menu exposes its additive version-1 template mutation capability, TPS Linter asks it on worker entry and again immediately before `Vault.process`; only a literal `true` permits the automatic write. A protected template, an unreadable current template source, a rejected check, or a malformed capability result leaves the note unchanged. Older GCM versions, a missing GCM installation, and GCM builds without that additive method retain the existing lint behavior. This handoff applies to the automatic Cmd-S/Ctrl-S and page-focus workflow only: **Check current note** stays read-only, while **Clean current note** remains an explicit user-authorized mutation even for a template.

Background plugin writes, Sync arrivals, metadata updates, autosave persistence, and other Vault modifications do not schedule TPS Linter, so they cannot turn into an implicit cleanup burst. Enabling the workflow does not enable individual cleanup rules; default-off rules such as plain-note body spacing, after-frontmatter body spacing, and list-item compaction must also be enabled locally or downloaded through same-remote-vault settings sync. A native top-right Obsidian notice briefly lists successfully applied changes. It names at most three cleanup actions, then gives the exact number of remaining fixes, and has a 180-character ceiling. An explicit conventional Cmd-S/Ctrl-S inside the active editor reports no changes under the rules enabled on that device, note-local disablement, or a safety block when it produces no mutation. Page-focus no-ops stay silent. Excluded, inactive, preview-only, unsaved-buffer, and GCM-protected template targets are skipped without a misleading success notice. There is no startup scan, batch mutation command, paste hook, or inactive-note sweep.
...
No startup scan occurs. Check and manual Clean operate only on the explicitly selected current note. Automatic linting queues only the current file from an in-editor conventional Save, focus entering the active source-mode Markdown page from outside its view, or the Obsidian window regaining focus while that page owns focus. Settings, modals, sidebars, internal focus moves, background Vault modifications, other views, inactive notes, preview-only notes, deleted, replaced, renamed-away, excluded, non-Markdown, newer-unsaved-buffer, and GCM-protected template targets are skipped. Template identity is not folder-dependent: the shared GCM rule can protect a template anywhere in the vault.

## Settings design

Check and Clean actions remain first. The always-visible **Choose what to configure** hub has four one-click destinations:

1. **Clean notes** — explicit-save/page-focus linting, body-start spacing, general and list-item blank lines, whitespace, and final newline;
...
The plugin has no network access, credentials, scheduled sweep, startup scan, Vault modification hook, create hook, rename hook, file-open hook, deletion, archive movement, or production deployment path. It registers one lifecycle-owned passive observer per open Obsidian workspace document for the standard save chord, focus entering an active Markdown page, and window focus returning to a page that already owns focus. The observer is mechanically non-canceling and never alters Obsidian's handlers. Main-window, existing-popout, restored-popout, and newly opened popout documents are covered; closed-popout observers are detached. TPS Linter does not replace or patch Obsidian's save command and schedules the guarded worker only when the event resolves to the active source-mode Markdown view.

Every manual Clean takes a fresh read, snapshots rule options, and enters one unconditional `Vault.process` callback. Manual Check/Clean inspection performs one initial safety scan and note-control parse for that exact revision and shares the result with Markdown cleanup. Only when an explicit Check/Clean or standard Save is otherwise a no-op, one additional combined read-only cleanup probe identifies applicable body-spacing or list-gap settings that are off; page-focus no-ops never pay that cost. When the process callback receives exactly the same bytes, it reuses the already verified preflight result; any byte difference, including BOM or line-ending representation, runs the original cleanup against the current revision. Post-clean note-control comparison and second-pass idempotence verification remain independent. Manual inspection and cleaning require the selected Markdown `TFile` to remain the exact object currently indexed by the vault before and after inspection reads, inside the process callback, after processing, and after the final filename read. A deleted and recreated same-path note never inherits content or filename work; the plugin does not retry, reacquire by path, or use adapter-level mutation. Same-file manual and automatic cleans are serialized.

The automatic scheduler debounces independently per note; an eligible trigger during a run requests one delayed rerun, worker failures cannot wedge its state, disabling the setting cancels pending timers, and plugin unload invalidates in-flight work before preventing reruns. The automatic worker rechecks enablement, plugin lifecycle, active editor/file identity, source mode, Markdown type, initial-plus-live exclusions, current rule settings, and that the editor buffer still matches the persisted revision at the asynchronous mutation boundaries. With the compatible GCM capability present, it also asks whether the exact `TFile` may be automatically mutated both before reading work into the run and immediately before the atomic process. A `false` decision, thrown or rejected call, or non-boolean result fails closed. Missing or capability-incompatible GCM retains the prior automatic behavior, and manual commands do not consult this background-mutation gate. Representation-only BOM and LF/CRLF/CR differences are tolerated; exact editor/file matches avoid normalization allocations. Real unsaved edits cause a no-write skip until a later eligible explicit-save or page-focus trigger. When both the persisted bytes and complete rule options remain identical at `Vault.process`, the verified pure cleanup result is reused instead of cleaning the same revision twice. Manual path exclusions are rechecked without relaxing the initial scope inside the callback, before filename planning, and after the final fresh read. Note-local controls and filename ownership are re-read from the final live content before manual rename eligibility. Markdown success is reported even if a later rename fails.
...
- Automatic template protection requires a loaded GCM build with the additive version-1 `templates.canAutomaticallyMutate` capability. Missing and older builds retain TPS Linter's previous behavior for compatibility. Manual Clean intentionally remains available when the user directly requests it.
...
- The conventional Cmd-S/Ctrl-S gesture can lint an unchanged open note because the key gesture is observable. A user-remapped or menu-based Save is not treated as an explicit trigger. Autosave and background Vault modifications intentionally do not lint; use the conventional chord or move focus from outside the active Markdown page back into it.
...
Validation covers the additive version-1 GCM template capability; literal allow/deny results; thrown, rejected, and malformed-result failure; missing/older capability compatibility; receiver and exact-file forwarding; two automatic mutation-boundary checks; and preservation of explicit manual Clean. The complete unit/property suite passed 273/273 tests, the focused compatibility subset passed 6/6, the structural/settings suite passed 18/18, and TypeScript and the separate production-mode build passed. Obsidian 1.13.7 loaded Linter 0.7.6 beside GCM 1.50.0 in the isolated test vault; Cmd-S left a deliberately noncanonical tagged template byte-identical. The fixture was archived, production was not accessed, and final hashes are recorded in `release-notes/0.7.6.md`.
...
Validation covers the absence of every Vault modification trigger; exact Cmd-S/Ctrl-S recognition; focus entering the active source-mode Markdown page from outside its view; ignored internal focus moves; main-window and popout window refocus; editor/document/target containment; silent focus no-ops; explicit-save no-change feedback; scheduler coalescing; lifecycle cleanup; unchanged manual commands, content rules, filename ownership, settings persistence, and atomic active-editor guards. Exact final counts, hashes, reload evidence, automatic-trigger QA, settings preservation, QA-note disposition, and production non-mutation are recorded in `release-notes/0.7.4.md`.
...
- Defers automatic Save/page-focus mutation permission to the loaded GCM version-1 template capability, so shared template identity protects sources outside conventional template folders.
- Checks permission on worker entry and again immediately before `Vault.process`; false, rejected, thrown, and malformed results fail closed.
- Keeps older/missing GCM behavior, every setting/default/schema/exclusion, and explicit manual Check/Clean behavior unchanged.

### 0.7.5

- Adds a final fail-closed frontmatter-envelope check so TPS Linter cannot return or write a changed note that alters or removes an existing top-of-note YAML opener or closer.
- Preserves exact `---` and `...` delimiter representation across BOM, LF, CRLF, and CR inputs while leaving already-unclosed notes unchanged.
- Changes no trigger, command, setting, rule default, schema field, exclusion, filename behavior, or minimum Obsidian requirement.

### 0.7.4

- Removes the automatic Vault `modify` trigger, so background plugin writes, Sync arrivals, metadata updates, and ordinary autosave persistence cannot schedule cleanup.
- Keeps the exact platform Cmd-S/Ctrl-S gesture as an explicit trigger when it originates inside the active source-mode Markdown editor.
- Adds passive page-focus triggering only when focus enters the active Markdown view from outside it, or when its Obsidian window regains focus while that page owns focus; internal focus moves do not retrigger.
- Preserves the existing `lintOnSave` setting key and default, content-only ownership, notices, scheduler, popout lifecycle, active-editor/file-revision guards, rules, commands, exclusions, settings sync, and minimum Obsidian requirement.
